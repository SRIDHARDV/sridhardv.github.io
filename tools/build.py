#!/usr/bin/env python3
"""Build the generated parts of the site from content/.
...
Writes:
    content/projects.json     ordered slug index, used by the home page grid
    sitemap.xml               one entry per project, always in step
    p/<slug>/index.html       a fully static page per project
...
Run it after adding or editing anything under content/projects/:
...
INDEX_OUT = ROOT / "content" / "projects.json"
...
def main() -> int:
    ...
    for path in sorted(SRC.glob("*.md")):
        ...
        warnings.extend(check_supported(path, body))
        projects.append({"slug": path.stem, "meta": meta, "body": body})
    ...
    if write_if_changed(INDEX_OUT, json.dumps(manifest, indent=2) + "\n"):
        changed.append("content/projects.json")

    # 2. Sitemap. Generated from the same list, so it cannot drift.
    urls = ["  <url><loc>%s/</loc><priority>1.0</priority></url>" % origin]
    urls += ["  <url><loc>%s/p/%s/</loc></url>" % (origin, p["slug"]) for p in projects]
    sitemap = (
    ...
    for i, project in enumerate(projects):
        ...
        out = PAGES_OUT / slug / "index.html"
        wanted.add(out.parent)
        if write_if_changed(out, page):
            changed.append("p/%s/index.html" % slug)

    # 4. Remove pages for projects that no longer exist.
    if PAGES_OUT.is_dir():
        for stale in sorted(d for d in PAGES_OUT.iterdir() if d.is_dir() and d not in wanted):
            shutil.rmtree(stale)
            changed.append("removed p/%s/" % stale.name)
